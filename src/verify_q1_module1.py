...
STEP_CHECKS = [
 ...
 ('m1-ord-n01', 0, 2.83, 43.8/15.5, 0.005, '43.8/15.5'),
 ...
 ('m1-ord-n11', 1, 0.1, -(0.30-0.45)/1.5, 0.0001, '0.15/1.5'),
 ('m1-ord-n11', 2, 0.5, 0.30+0.1*2.0, 0.0001, '0.30+0.20'),
 ('m1-ord-n11', 3, 600, 300/0.5, 0.0001, '300/0.5'),
...
rows = list(STEP_CHECKS)
rows.sort(key=lambda r: (r[0], r[1]))
def forms(p):
    """The spellings a printed value may take in the text: as written, and
    with trailing decimal zeros dropped. Only a decimal is trimmed, since
    str(600).rstrip('0') is '6' and would match almost anything."""
    s = str(p)
    return {s, s.rstrip('0').rstrip('.')} if '.' in s else {s}

def found_in(qid, i, printed):
    """The index of the step whose text carries the value: the named step
    first, then its neighbours, so a value that moved one line when a setup
    step was added is reported by where it is rather than as missing."""
    steps = BY[qid]['steps']
    def hit(step):
        blob = (step['t'] + ' ' + step['why']).replace(',', '')
        return any(f in blob for f in forms(printed))
    if hit(steps[i]): return i
    return next((j for j, st in enumerate(steps) if hit(st)), None)

for qid, i, printed, mine, tol, lab in rows:
    step = BY[qid]['steps'][i]
    where = found_in(qid, i, printed)
    ...
    note = '' if where == i else (f'   (in step {where+1})' if present else '   <-- NOT IN STEP TEXT')
    print(f"  {'ok ' if ok and present else 'FAIL'} {qid} step {i+1} [{step['k']:<7}] "
          f"{lab:<22} printed {printed!s:<11} recomputed {mine:.6g}{note}")
