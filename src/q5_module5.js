...
{id:'m5-c23', prof:'Mosley', tier:'new', exam:2, module:5, lecture:'L06',
 ...
 teach:[
  {t:'Raising ka changes only how fast drug gets into the body, not how much gets in or how fast it leaves. Each of the three results follows from one of the two definitions below.'},
  {h:'Two definitions to hold first', list:['tmax is the moment the rate of drug coming in equals the rate of drug going out. Before it, more arrives than leaves, so the level rises; after it, more leaves than arrives, so the level falls.', 'AUC is the total exposure the absorbed dose produces before it is removed. AUC = {{frac:F x D0|Cl}}, and Cl = k x VD.']},
  {h:'Raising ka, one result at a time', list:['Cmax is higher. Drug arrives faster than elimination can remove it, so the concentration builds higher before the two rates meet.', 'tmax is earlier. Faster absorption brings the rate in and the rate out level with each other sooner, so the peak comes sooner.', 'AUC is unchanged. The dose and F set how much gets in, and clearance, k x VD, sets how fast it leaves. ka is in neither, so total exposure stays where it was.']},
  {h:'Raising ka against raising k', table:{head:['', 'ka raised, k the same', 'k raised, ka the same'], rows:[['Cmax', 'Higher: drug arrives faster than it can leave', 'Lower: drug leaves faster while it is still arriving'], ['tmax', 'Earlier: the rate in meets the rate out sooner', 'Earlier: the rate out catches up with the rate in sooner'], ['AUC', 'Unchanged: clearance does not move', 'Lower: clearance rises, since Cl = k x VD'], ['What changed', 'The input side', 'The output side']]}, fig:'k_vs_ka', after:'The figure uses the values on her slide: a 100 mg dose, a volume of distribution of 10 L, and the constant that is not being changed held at 0.1 hr^-1. Raising k from 0.2 to 0.5 hr^-1 lowers the peak from 2.50 to 1.34 µg/mL, moves it from 6.9 to 4.0 hours, and cuts the AUC from 50 to 20 µg·hr/mL. Raising ka over the same values raises the peak from 5.00 to 6.69 µg/mL, moves it from 6.9 to 4.0 hours, and leaves the AUC at 100 µg·hr/mL. tmax moves earlier in both, so tmax on its own cannot tell you which constant changed; Cmax and AUC can.'}],
...
{id:'m5-c24', prof:'Mosley', tier:'new', exam:2, module:5, lecture:'L06',
 ...
 teach:[
  {t:'Raising k changes only how fast drug leaves the body. Each of the three results follows from one of the two definitions below.'},
  {h:'Two definitions to hold first', list:['tmax is the moment the rate of drug coming in equals the rate of drug going out. Before it, more arrives than leaves, so the level rises; after it, more leaves than arrives, so the level falls.', 'AUC is the total exposure the absorbed dose produces before it is removed. AUC = {{frac:F x D0|Cl}}, and Cl = k x VD.']},
  {h:'Raising k, one result at a time', list:['Cmax is lower. Drug is being removed faster while it is still arriving, so the concentration cannot build as high.', 'tmax is earlier. Faster removal brings the rate out up to the rate in sooner, so the peak comes sooner.', 'AUC is lower. Raising k raises clearance (Cl = k x VD), so the same absorbed dose spends less time in the plasma and produces less total exposure.']},
  {h:'Raising k against raising ka', table:{head:['', 'k raised, ka the same', 'ka raised, k the same'], rows:[['Cmax', 'Lower: drug leaves faster while it is still arriving', 'Higher: drug arrives faster than it can leave'], ['tmax', 'Earlier: the rate out catches up with the rate in sooner', 'Earlier: the rate in meets the rate out sooner'], ['AUC', 'Lower: clearance rises, since Cl = k x VD', 'Unchanged: clearance does not move'], ['What changed', 'The output side', 'The input side']]}, fig:'k_vs_ka', after:'The figure uses the values on her slide: a 100 mg dose, a volume of distribution of 10 L, and the constant that is not being changed held at 0.1 hr^-1. Raising k from 0.2 to 0.5 hr^-1 lowers the peak from 2.50 to 1.34 µg/mL, moves it from 6.9 to 4.0 hours, and cuts the AUC from 50 to 20 µg·hr/mL. Raising ka over the same values raises the peak from 5.00 to 6.69 µg/mL, moves it from 6.9 to 4.0 hours, and leaves the AUC at 100 µg·hr/mL. tmax moves earlier in both, so tmax on its own cannot tell you which constant changed; Cmax and AUC can.'}],
 teachImg:'slide_5Pharmacokineticso_p19',
 cite:'5---Pharmacokinetics-of-Oral-Absorption.pdf, slide "Effect of ka and k on Cmax, tmax, and AUC"; 09.21 lecture',
